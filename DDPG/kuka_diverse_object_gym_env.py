...
class KukaDiverseObjectEnv(KukaGymEnv):
  ...
  def _randomly_place_objects(self, urdfList):
    ...
    for urdf_name in urdfList:
      ...
      for _ in range(500):
        p.stepSimulation()
    # embed()
    self.object_index = objectUids[0]
    return objectUids
  ...
  def _step_continuous(self, action):
    ...
    for _ in range(self._actionRepeat):
      # print('_env_step=', self._env_step)
      p.stepSimulation()
      if self._renders:
    ...
    # reward = self._reward()
    reward, reward_train = self._reward()

    debug = {
        'grasp_success': self._graspSuccess
    }
    return observation, reward, reward_train, done, debug
  ...
  def get_reward(self, obj_xy, new_goal_xy, obj_z):
    reward_train, reward = 0, 0
    
    # INTRINSIC REWARD
    # Simple Intrinsic Reward
    pos_z = "%.2f" % obj_z
    if self.state_buffer[pos_z] != 0:
      reward_train += 1/np.sqrt(self.state_buffer[pos_z])
    else:
      reward_train += 2

    # More Complex Intrinsic Reward
    # distance = np.sqrt(obj_z[0]**2 + obj_z[1]**2)
    # pos_z = "%.2f" % distance + " " + "%.2f" % pos[2]
    # if self.state_buffer[pos_z] != 0:
    #   reward_train += 1/np.sqrt(self.state_buffer[pos_z])
    # else:
    #   reward_train += 2
    ...
    # HINDSIGHT REWARD
    obj_xy, new_goal_xy = np.array(obj_xy), np.array(new_goal_xy)
    reward_train -= np.linalg.norm(obj_xy - new_goal_xy)
  ...
  def _reward(self):
    ...
    for uid in self._objectUids:
      ...
      # Simple Intrinsic Reward
      pos_z = "%.2f" % pos[2]
      if pos_z not in self.state_buffer:
        self.state_buffer[pos_z] = 0
      else:
        self.state_buffer[pos_z] += 1
      if self.state_buffer[pos_z] != 0:
        reward_train += 1/np.sqrt(self.state_buffer[pos_z])
      else:
        reward_train += 2
      ...
      # HINDSIGHT REWARD
      pos_xy = np.array([pos[0]-end_effector_pos[0], pos[1]-end_effector_pos[1]])
      distance_xy = np.linalg.norm(pos_xy - np.array([0, 0]))
      reward_train -= distance_xy
      
      # REGULAR TASK REWARD
      # If any block is above height, provide reward.
      if pos[2] > 0.2:
        self._graspSuccess += 1
        print('Got Task Reward / Task Success')
        reward_train += 1
        reward = 1
        break
    return reward, reward_train
